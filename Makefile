...
default:
	...
	sh -c "sed '$ d' $(KERNELDIR)/scripts/module.lds.S > $(KERNELDIR)/scripts/module.lds"
...
clean:
	...
	sh -c "rm -f $(KERNELDIR)/scripts/module.lds"
...
dkmsclean:
	...
	dkms remove -m hid-asus-rog -v 1.0.0 --all || true
	dkms remove -m hid-asus-rog -v 1.0.1 --all || true

dkms: dkmsclean
	dkms add .
	dkms install -m hid-asus-rog -v 1.0.1
